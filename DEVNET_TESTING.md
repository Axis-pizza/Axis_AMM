# Axis Protocol — Devnet Testing Guide

> **Scope note.** This guide covers the **full devnet stack** including the academic A/B comparison. **Mainnet v1 ships only `pfda-amm-3` + `axis-vault`** — `axis-g3m` and `pfda-amm` (legacy) stay on devnet only. See [`docs/architecture/MAINNET_SCOPE.md`](docs/architecture/MAINNET_SCOPE.md) for the audit-ready scope spec.

## What This Is

Four on-chain programs deployed to Solana devnet. The first two are the mainnet v1 stack; the other two are research-only baselines that produce the A/B comparison data in `reports/ab/*.md`.

- **`pfda-amm-3` (ETF A, mainnet v1)** — batch auction with Switchboard oracle bounding + Jito bid; the LVR-mitigation product.
- **`axis-vault` (mainnet v1)** — ETF token lifecycle: deposit basket → mint, burn → withdraw basket. Includes `DepositSol` / `WithdrawSol` Jupiter CPI flows.
- **`axis-g3m` (ETF B, devnet only)** — 5-token G3M continuous AMM, the academic baseline that loses to PFDA in the A/B test. Not shipped to mainnet in v1.
- **`pfda-amm` (legacy, devnet only)** — original 2-token prototype; kept as regression coverage.
...
There are TWO devnet environments:

### Canonical (Muse-owned)

The original deployment used in `reports/ab/*` and the public docs.

| Program | ID | Role | Mainnet v1? |
|---|---|---|---|
| **pfda-amm-3** | `DbAPmgkrpCCZrpBMv5x1ye6nJUreqY313SuQjZsMyjEf` | 3-token PFDA batch auction with Switchboard + Jito | ✅ |
| **axis-vault** | `DeeUnCHcnPG8arbjGTLhTKeDhpPUBper3TDrpFPHnCwy` | ETF token lifecycle | ✅ |
| **axis-g3m** | `65aE9QdVz5bapV19BGt5cyTgVitYpekGwusRoQEovNUi` | 5-token G3M AMM (research baseline) | ❌ |
| **pfda-amm** (legacy) | `CSBgQGeBTiAu4a9Kgoas2GyR8wbHg5jxctQjq3AenKk` | Original 2-token prototype | ❌ |

Upgrade authority: `6t4B1TVgSjnAM9h5MpahLhGc9MtWFTGmcaPsy9JGskoV`. Most teammates do not have this key on disk.

### Kidney's parallel environment

Generated by `scripts/ops/deploy-devnet.sh --fresh`. The IDs live in `.env.devnet.kidney` (gitignored). Run the e2e suite against this environment by `source`ing the env file and prefixing each test with `PROGRAM_ID=$<NAME>_PROGRAM_ID`. See the canonical IDs file for the current snapshot.
