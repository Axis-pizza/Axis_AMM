...
pub fn build_pfda3_pool_state(
    ...
    for i in 0..3 { let o = 8 + i * 32; d[o..o + 32].copy_from_slice(mints[i].as_ref()); }  // 8: mints
    for i in 0..3 { let o = 104 + i * 32; d[o..o + 32].copy_from_slice(vaults[i].as_ref()); } // 104: vaults
    for i in 0..3 { let o = 200 + i * 8; d[o..o + 8].copy_from_slice(&reserves[i].to_le_bytes()); } // 200: reserves
    for i in 0..3 { let o = 224 + i * 4; d[o..o + 4].copy_from_slice(&weights[i].to_le_bytes()); }  // 224: weights
...
pub fn build_batch_queue_3(
    ...
    let mut off = 0;
    d[off..off + 8].copy_from_slice(b"batch3q\0"); off += 8;
    d[off..off + 32].copy_from_slice(pool.as_ref()); off += 32;
    d[off..off + 8].copy_from_slice(&batch_id.to_le_bytes()); off += 8;
    for i in 0..3 { d[off..off + 8].copy_from_slice(&total_in[i].to_le_bytes()); off += 8; }
    d[off..off + 8].copy_from_slice(&window_end_slot.to_le_bytes()); off += 8;
    d[off] = bump;
    d
}
